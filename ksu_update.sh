...
git subtree pull --prefix=drivers/staging/kernelsu https://github.com/tiann/KernelSU.git v0.9.5 --squash -m "drivers: kernelsu: update"

# Lets tricky calculating for the reall version here
rm -rf ksu
git clone https://github.com/tiann/KernelSU ksu -b v0.9.5
cd ksu
KSU_VERSION=10200
...
sed -i "s/$OLD_VER/$NEW_VER/g" drivers/staging/kernelsu/kernel/Makefile

echo ""
echo -e "${green}KernelSU has been Updated"
echo -e "${restore}Curent version is ${blink_red}$CURRENT"
echo "$CURRENT" > drivers/staging/kernelsu/kernel/dksu
echo ""
echo "Adding commit message:"

git add drivers/staging/kernelsu/kernel/Makefile drivers/staging/kernelsu/kernel/dksu
